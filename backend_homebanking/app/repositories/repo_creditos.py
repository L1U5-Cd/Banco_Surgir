...
def listar_solicitudes_cliente(conn: Connection, pkcliente: int, limit: int = 20) -> list[dict]:
    """Solicitudes del cliente desde la misma dsolicitud que usa el core."""
    rows = conn.execute(
        text(
            """
            SELECT s.pksolicitud,
                   s.pkcliente,
                   TRIM(s.codsolicitud) AS codsolicitud,
                   s.montosolicitudcredito AS montosolicitud,
                   s.montoaprobadocredito AS montoaprobado,
                   s.plazosolicitudcredito AS plazo,
                   s.nrocuotasolicitud AS nrocuotas,
                   s.fechasolicitudcredito AS fecha_solicitud,
                   s.fechaaprobacioncredito AS fecha_aprobacion,
                   s.fechahoraultmodificacion AS fecha_actualizacion,
                   e.dessolicitudestado AS estado,
                   TRIM(e.codsolicitudestado) AS codestado,
                   s.desmotivosolicitud AS observacion
            FROM dsolicitud s
        ...
        {"pkcliente": pkcliente, "limit": limit},
    ).mappings().all()
